...
def draw_route_map(stop_data, route_info):
    ...
    if is_circular:
        ...
        # ==================== [핵심 수정 부분 시작] ====================
        # 종점 마커 (빨간색) - 상행 경로의 마지막 정류장이 아닌, 공식 종점 이름으로 검색
        if outbound_stops:
            # 기본 종점을 상행 경로의 마지막 정류장으로 설정 (fallback용)
            end_point = outbound_stops[-1]
            
            # API에서 제공하는 공식 종점 이름으로 정류장 리스트에서 검색
            official_end_name = route_info['end']
            found_end_stops = [stop for stop in outbound_stops if stop['name'] == official_end_name]
            
            # 공식 종점 이름과 일치하는 정류장을 찾았다면, 그 정류장을 진짜 종점으로 사용
            if found_end_stops:
                end_point = found_end_stops[0] # 일치하는 정류장들 중 첫 번째 것을 사용
    ...
    color_map = {
        '간선버스': 'blue', '광역버스': 'red', '지선버스': 'green',
        '첨단버스': 'skyblue', '급행버스': 'orangered', '심야버스': 'black'
    }
    ...
    <style>
     .info-box {{
         ...
         box-shadow: 3px 3px 5px rgba(0,0,0,0.3); font-family: Arial, sans-serif;
         font-size: 14px; line-height: 1.6; max-width: 300px;
         border: 2px solid {border_color};
     }}
     .info-box h4 {{
         margin: -10px -10px 10px -10px; padding: 5px 10px; font-size: 18px;
         font-weight: bold; background-color: {border_color}; color: white;
